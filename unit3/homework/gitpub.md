...
- set a variable of `port` to `5000`
- set your app to `listen` to the port and include a `console.log()`, so that you can tell when your server is running
- include a get route / that will `res.send('Welcome to the Gitpub App!');` so that when you got to `localhost:5000`, you will see `Welcome to the Gitpub App!`
...
- go to `localhost:5000` (or whatever port you set it to)
...
- In your browser, go to `localhost:5000/drinks` to make sure you're rendering the `index.ejs` file! 
...
- For now, just make sure it works correctly by havine the route `res.send(req.params.id)` 
  - So that when you go to `localhost:5000/drinks/whatever`, `whatever` should show up in the browser 
...
- Submit your homework via github issues before the next class starts at 9.30AM and please don't forget to fill out the form!
