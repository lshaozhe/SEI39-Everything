...
const createPlant = async (req, res) => {
  try {
    ...
    const results = await Plants.findOne({ name: req.body.name });
    res.json(results);
  } catch (err) {
...
const getPlantByID = async (req, res) => {
  try {
    const results = await Plants.findById(req.params.id);
    res.json(results);
  } catch (err) {
    console.log("GET /api/plant/id", error);
    res.status(400).json({ status: "error", message: "an error has occured" });
  }
};
...
const getAllPlants = async (req, res) => {
  try {
    const results = await Plants.find({}, { id: 1 });
    res.json(results);
  } catch (err) {
    console.log("GET /api/all", error);
    res.status(400).json({ status: "error", message: "an error has occured" });
  }
};
...
const updatePlant = async (req, res) => {
  try {
    await Plants.findByIdAndUpdate(req.params.id, {
      name: req.body.name,
      description: req.body.description,
      image: req.body.image,
    });
    res.json({ status: "ok", message: "updated one" });
  } catch (err) {
    console.log("PUT /api/update/", error);
    res.status(400).json({ status: "error", message: "an error has occured" });
  }
};
...
const deletePlant = async (req, res) => {
  try {
    ...
    res.json({ status: "ok", message: "deleted one" });
  } catch (err) {
...
const seedPlants = async (req, res) => {
...
module.exports = { seedPlants, createPlant, getAllPlants, getPlantByID, updatePlant, deletePlant };
