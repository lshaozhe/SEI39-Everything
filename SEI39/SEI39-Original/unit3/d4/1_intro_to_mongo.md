...
There are many databases. A common type is a _SQL_(Structured Query Language) database which stores data in tables and rows, much like an excel spreadsheet/google sheet.

Another type of database is a NoSQL(Not Only _SQL_) database, which follows a different structure. In our case, we'll be using MongoDB which will store our data in objects (just as we've been seeing with our mock databases)
...
[Our Cheatsheet](https://developer.mongodb.com/quickstart/cheat-sheet/)
...
Let's use three :three: terminal tabs. One :one: to keep bash open and available, open a new one :one: to start `mongod` and one :one: more to run the mongo shell.
...
Let's use our `UP arrow` :arrow_up: to scroll back to our .find() for Jennifer and check that we now have just one record.
...
We can push the up arrow :arrow_up: to rerun
...
Press the up arrow :arrow_up: to run
