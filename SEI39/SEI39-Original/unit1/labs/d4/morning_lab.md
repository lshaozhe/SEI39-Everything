...
Remember, objects are a type of **data structure** that we use to store data with greater specificity than arrays. The greater specificity is achieved becaue objects hold what are called **key-value pairs**.
...
Note that the objects do not have names. They are just anoynmous objects listed in an array. 
