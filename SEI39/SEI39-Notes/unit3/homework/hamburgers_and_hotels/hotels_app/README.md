...
- either run with `node server.js` or start nodemon (caution - every time you save the server will restart, possibly causing your commands to go through more times than you'd want)
 - Expected output:
...
- require `models/seed.js` as `hotelSeed` (still working inside your `server.js`)
- Add the following below `db.once('open', ...`
...
#### With each prompt, complete it, then comment it out - write all your code inside `server.js`
[Mongo Documentation](https://docs.mongodb.com/manual/crud/)

### C
- Create a hotel using the above schema
  - add your mongoose command inside server.js. Once it works, comment it out.
...
- install express
- create an app.get route that `res.json` all the hotels to the browsers to be viewed as json
