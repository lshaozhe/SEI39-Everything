...
-- Clue #1: We recently got word that someone fitting Carmen Sandiego's description has been traveling through Southern Europe. She's most likely traveling someplace where 
-- she won't be noticed, so find the least populated country in Southern Europe, and we'll start looking for her there.
...
-- Clue #3: We have new news on the classes Carmen attended – our gumshoes tell us she's moved on to a different country, a country where people speak only the language 
...
-- Clue #5: Oh no, she pulled a switch – there are two cities with very similar names, but in totally different parts of the globe! She's headed to South America as we speak; 
-- go find a city whose name is like the one we were headed to, but doesn't end the same. Find out the city, and do another search for what country it's in. Hurry!

SELECT * FROM city
JOIN country ON country.code=city.countrycode
WHERE city.name LIKE 'Ser%'
AND country.region='South America';



-- Clue #6: We're close! Our South American agent says she just got a taxi at the airport, and is headed towards the capital! Look up the country's capital, and get there pronto! 
-- Send us the name of where you're headed and we'll follow right behind you!

SELECT city.name FROM city
JOIN country ON country.code=city.countrycode
WHERE country.code='BRA' AND
...
SELECT name, countrycode from public.city WHERE population = 91084;
