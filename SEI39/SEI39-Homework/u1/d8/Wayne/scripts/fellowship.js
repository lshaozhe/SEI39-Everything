...
const keepItSecretKeepItSafe = () => {
  ...
  theRing.id = "the-ring";
  const listHobbits = document.querySelectorAll(".hobbit");
  listHobbits[0].append(theRing);
};
...
const makeBuddies = () => {
  ...
  ulBuddies.id = "ulBuddies";

  function createBuddiesLi(buddiesName) {
    const buddiesLi = document.createElement("LI");
    buddiesLi.innerText = buddiesName;
    buddiesLi.className = "buddy";
    ulBuddies.append(buddiesLi);
  }

  function generateBuddiesLi() {
    for (i = 0; i < buddies.length; i++) {
      createBuddiesLi(buddies[i]);
    }
  }
...
const beautifulStranger = () => {
  ...
  const buddyLi = document.querySelectorAll(".buddy");
  console.log(buddyLi);
  buddyLi[3].innerText = "Aragorn";
};
