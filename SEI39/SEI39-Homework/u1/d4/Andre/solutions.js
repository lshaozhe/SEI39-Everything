...
delete inception.reality.dreamLayer1.dreamLayer2.dreamLayer3.dreamLayer4
...
let bondTitles = [];
for (const items of bondFilms) {
  bondTitles.push(items.title);
}
console.log(bondTitles);
let oddBonds = [];
for (const items of bondFilms) {
  if (items.year % 2 !== 0) {
    oddBonds.push(items.title);
  }
}
...
let cumulGross = 0;
for (const items of bondFilms) {
  grossi = items.gross.replace(/,/g, "");
  grossi = grossi.replace(/\$/g, "");
  //console.log(grossi);
  cumulGross += parseInt(grossi);
}
...
const bondActors = {};
for (const items of bondFilms) {
  bondActors[items.actor] = 0;
}
//console.log(bondActors);
for (const items of bondFilms) {
  for (const keys in items) {
    if (items[keys] in bondActors) {
      bondActors[items[keys]] += 1;
    }
  }
}
//console.log(bondActors);
let theActor = "";
for (const keys in bondActors) {
  if (bondActors[keys] === 1) {
    theActor = keys;
  }
}
for (const items of bondFilms) {
  if (items.actor === theActor) {
    console.log(items);
  }
}
